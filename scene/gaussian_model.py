...
from scene.network import Temp_TimeNet, ConvectiveHeatTransferNet, EmissivityNet, HeatCapacityNet, Temp_Time_DerivativeNet
...
class GaussianModel:
    def __init__(self, sh_degree : int, brdf_dim : int, brdf_mode : str, brdf_envmap_res: int, feature_time: bool):
        ...
        # themal setting
        # winter days 12w points step_n = 12
        # 18:44:21 - 24:44:18
        # self.time_interval = 21597.0
        # self.min_temp = -20.0 + 273.15
        # self.max_temp = 20.0 + 273.15
        # self.space_feature = torch.empty(0)
        # self.env_temp = torch.tensor(((0.0 + 273.15) - self.min_temp) / (self.max_temp - self.min_temp)).float().unsqueeze(0).unsqueeze(0).unsqueeze(0).cuda()
        # self.wind_speed = torch.tensor(17.0 / 50.0).float().unsqueeze(0).unsqueeze(0).unsqueeze(0).cuda()
        
        # summer 0707 s1:70w points step_n = 6  s2s4s5:40w points step_n = 6 
        self.time_interval = 16004 #s1=16004 s2=16857 s4=7789 s5=8099
        self.min_temp = 10 + 273.15
        self.max_temp = 50 + 273.15
        self.space_feature = torch.empty(0)
        ...
        # summer 0909 s1:70w points step_n = 6  s2:40w points step_n = 6 
        # self.time_interval = 16004 #s1=16004
        # self.min_temp = 10 + 273.15 #s1=0 s2=0 s3=0 s4=0
        # self.max_temp = 50 + 273.15 #s1=70 s2=70 s3=70 s4=70
        # self.space_feature = torch.empty(0)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, space_features : torch.tensor, spatial_lr_scale : float):
    ...
    def training_setup(self, training_args):
        ...
        if self.feature_time:
            l.extend([
                {'params': list(self.Temp_TimeNet.parameters()), 'lr': training_args.brdf_mlp_lr_init / 10, "name": "Temp_TimeNet"},
                # {'params': list(self.ConvectiveHeatTransferNet.parameters()), 'lr': training_args.brdf_mlp_lr_init, "name": "ConvectiveHeatTransferNet"},
                # {'params': list(self.EmissivityNet.parameters()), 'lr': training_args.brdf_mlp_lr_init, "name": "EmissivityNet"},
                # {'params': list(self.HeatCapacityNet.parameters()), 'lr': training_args.brdf_mlp_lr_init, "name": "HeatCapacityNet"},
                {'params': list(self.Temp_Time_DerivativeNet.parameters()), 'lr': training_args.brdf_mlp_lr_init, "name": "Temp_Time_DerivativeNet"},
    ...
    def save_net(self, path):
        save_file = {"Temp_TimeNet": self.Temp_TimeNet.state_dict(),
                    # "ConvectiveHeatTransferNet": self.ConvectiveHeatTransferNet.state_dict(),
                    # "EmissivityNet": self.EmissivityNet.state_dict(),
                    # "HeatCapacityNet": self.HeatCapacityNet.state_dict(),
                    'Temp_Time_DerivativeNet': self.Temp_Time_DerivativeNet.state_dict(),
                    "space_feature": self.space_feature}    
        torch.save(save_file, path)
    def reset_opacity(self):
        opacities_new = inverse_sigmoid(torch.min(self.get_opacity, torch.ones_like(self.get_opacity)*0.01))
        optimizable_tensors = self.replace_tensor_to_optimizer(opacities_new, "opacity")
        self._opacity = optimizable_tensors["opacity"]
    def load_net(self, path, space_features):
        ckpt = torch.load(path)
        self.Temp_TimeNet.load_state_dict(ckpt['Temp_TimeNet'])
        # self.ConvectiveHeatTransferNet.load_state_dict(ckpt['ConvectiveHeatTransferNet'])
        # self.EmissivityNet.load_state_dict(ckpt['EmissivityNet'])
    ...
    def load_ply(self, path, og_number_points=-1):
        ...
        if self.feature_time:
            self._features_dc = torch.tensor(features_dc, dtype=torch.float, device="cuda").transpose(1, 2).contiguous()if not self.brdf else nn.Parameter(torch.tensor(features_dc, dtype=torch.float, device="cuda").requires_grad_(True))
        else:
