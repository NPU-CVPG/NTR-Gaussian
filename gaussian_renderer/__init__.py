...
def render(viewpoint_camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, override_color = None, debug=False, speed=False):
    ...
    if colors_precomp is None:
        if pipe.brdf:
            ...
            # shs = pc.get_features
            colors_precomp = pc.get_features.squeeze()
    else:
    ...
    if not speed:
        ...
        if pc.ConvectiveHeatTransfer.numel() != 0:
            render_extras.update({
                        "HeatTransfer": ((pc.ConvectiveHeatTransfer - pc.ConvectiveHeatTransfer.min()) / (pc.ConvectiveHeatTransfer.max() - pc.ConvectiveHeatTransfer.min())).repeat(1, 3).to(device='cuda:0'), 
                        "HeatCapacity": ((pc.HeatCapacity - pc.HeatCapacity.min()) / (pc.HeatCapacity.max() - pc.HeatCapacity.min())).repeat(1, 3).to(device='cuda:0'), 
                        "Emissivity": ((pc.Emissivity - pc.Emissivity.min()) / (pc.Emissivity.max() - pc.Emissivity.min())).repeat(1, 3).to(device='cuda:0'),  
                        })
        out_extras = {}
